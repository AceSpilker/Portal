...
>
> **v0.7 变更（需求变更）**：系统定位调整为**桌面 Web 专用**——§6.3 移动端 UI 方案废弃，移动端适配（底部 Tab/安全区/触控目标）移除，保留窄窗口侧栏折叠；PWA 调整为桌面浏览器安装场景。
>
...
| 缓存/会话（可选） | **Redis**（redis.asyncio）+ 进程内存降级实现 | 未配置 Redis 时自动回退进程内存（dev-plan P25） |
...
系统默认**零外部依赖**（SQLite + 进程内存）；客户机已有的 MySQL / Redis 服务只需在「系统配置」中填连接信息即可启用，全部可选、可降级。

| 层 | 未配置（默认） | 配置后 | 用途 |
|---|---|---|---|
| 运行主库 | **SQLite**（data/portal.db） | 不变（MySQL 仅作镜像，不承载运行读写） | 全部业务数据读写 |
| 灾备镜像 | 不启用 | **MySQL**：按可配置间隔定时全量 upsert（删除对齐、敏感表排除）；支持"立即推送"与"从 MySQL 恢复到 SQLite"（恢复前自动备份本地） | 灾备 / 集中存储 / 多实例汇总 |
| 会话存储 | 进程内存（重启丢失） | **Redis**：登录会话清单、登出黑名单、登录限速计数、传输加密会话密钥；重启后会话保持 | 登录态与安全计数 |
| 高速缓存 | 直接回源 | **Redis**：统一 CacheStore 接口（get/set/ttl/del）；监控实时数据、AI 上下文等热点数据按需接入 | 高频读加速 |

**降级与自检**：MySQL/Redis 连接参数存于系统配置（密码加密存储，支持环境变量覆盖）；中间件/定时任务对连接失败做退避与降级，恢复后自动回切/补推；健康自检（M15-10）输出各外部服务的连通性与当前存储模式。

**配置项**（系统配置 → 新增分组，均可选）：

- MySQL：host / 端口 / 账号 / 密码（加密存储）/ 目标库 / 推送间隔（分钟）/ 开关 / 立即推送 / 测试连接；
- Redis：host / 端口 / 密码 / DB / key 前缀 / 开关 / 测试连接。
...
登录后统一框架：左侧可收起导航栏（首页/应用/监控/Flow/AI/设置）+ 顶栏（全局搜索、**当前网络环境切换器 ★**、通知铃铛、AI 入口、用户菜单）。桌面 Web 专用（v0.7）：窄窗口时侧栏折叠为图标栏。

### 6.2b 系统在线更新策略（v0.7 新增）

**版本源**：Gitee Releases（`/api/v5/repos/{owner}/{repo}/releases/latest`，tag 即版本，body 即更新日志；仓库经设置键 `update.repo` 可配）。本地版本 = 构建注入（/api/health 已返回）。

**流程**：定时（6h）+ 手动检查 → 有新版则站内通知管理员 → 管理员确认执行 → 更新前自动备份（SQLite 全量导出，联动 P17.3）→ 按部署形态更新 → 健康自检（/api/health + 版本比对）→ 成功/失败回滚。

| 部署形态 | 更新方式 | 回滚 |
|---|---|---|
| 源码部署（git clone） | 系统内一键：`git fetch --tags` → checkout 新 tag → `pip install` / 前端构建 → 进程重启（init_db 轻量迁移自动执行） | checkout 旧 tag + 恢复更新前备份 |
| Docker 部署（NAS 主形态） | 默认半自动：提示新版 + 展示/复制更新命令（`docker compose pull && up -d`，数据卷保留历史不丢）；挂载 docker.sock（P12 开启）后可容器内一键拉取重建 | compose 指定旧镜像 tag 重建 + 数据卷不受影响 |

**安全约束**：仅管理员；默认不静默自动更新（`update.auto_check` 开、`update.auto_apply` 关）；更新过程前端全屏遮罩轮询健康接口；更新期间写审计日志。

### 6.3 移动端 UI 方案（v0.3 定稿；**v0.7 废弃**——需求变更：系统定位桌面 Web 专用，移动端适配已移除，仅保留窄窗口侧栏折叠）
